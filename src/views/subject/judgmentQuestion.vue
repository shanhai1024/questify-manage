<template>
  <div class="container">
    <h2 class="title">判断题管理</h2>

    <!-- 添加题目按钮 -->
    <div style="margin-bottom: 20px; text-align: right;">
      <!-- 按钮点击后打开添加判断题的对话框 -->
      <el-button type="success" @click="openAddDialog">添加判断题</el-button>
    </div>
    ...
    <!-- 编辑判断题弹出框 -->
    <el-dialog v-model="editDialogVisible" width="60%" title="编辑判断题" custom-class="edit-dialog">
      <el-form :model="editForm" label-width="120px" ref="editFormRef" :rules="editFormRules">
        ...
        <!-- 正确答案 -->
        <el-form-item label="答案" prop="isCorrect">
          <el-radio-group v-model="editForm.isCorrect">
    ...
    <!-- 添加判断题弹出框 -->
    <el-dialog v-model="addDialogVisible" width="60%" title="添加判断题" custom-class="edit-dialog">
      <el-form :model="addForm" label-width="120px" ref="addFormRef" :rules="addFormRules">
        ...
        <!-- 正确答案 -->
        <el-form-item label="答案" prop="isCorrect">
          <el-radio-group v-model="addForm.isCorrect">
            <el-radio :label="true">正确</el-radio>
            <el-radio :label="false">错误</el-radio>
          </el-radio-group>
        </el-form-item>
...
import { ElTable, ElTableColumn, ElButton, ElPagination, ElDialog, ElMessage, ElForm, ElFormItem, ElInput, ElSelect, ElOption, ElInputNumber, ElRadioGroup, ElRadio } from 'element-plus';
...
export default {
  ...
  setup() {
    ...
    const getSubjectData = async () => {
      try {
        // 调用接口获取判断题数据
        const response = await api.getAllByType(3, currentPage.value, pageSize.value);
    ...
    const editFormRules = {
      ...
      isCorrect: [{ required: true, message: '请选择正确答案', trigger: 'change' }],
      ...
      try {
        ...
        const requestData = {
          ...
          judge: {
            ...
            isCorrect: editForm.value.isCorrect ? 1 : 0
          }
        };

        // 调用接口更新数据
        const response = await api.updateSubject(3, requestData);
        if (response.code === 200) {
    ...
    const submitAdd = async () => {
      try {
        ...
        const requestData = {
          ...
          questionTypeCode: 3,
          judge: {
            isCorrect: addForm.value.isCorrect ? 1 : 0
          }
        };
