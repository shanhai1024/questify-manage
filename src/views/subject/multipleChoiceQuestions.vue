<template>
  <div class="container">
    <h2 class="title">多选题管理</h2>

    <!-- 添加题目按钮 -->
    <div style="margin-bottom: 20px; text-align: right;">
      <el-button type="success" @click="openAddDialog">添加多选题</el-button>
    </div>
    ...
    <!-- 编辑多选题弹出框 -->
    <el-dialog v-model="editDialogVisible" width="60%" title="编辑多选题" custom-class="edit-dialog">
    ...
    <!-- 添加多选题弹出框 -->
    <el-dialog v-model="addDialogVisible" width="60%" title="添加多选题" custom-class="edit-dialog">
...
export default {
  ...
  setup() {
    ...
    const getSubjectData = async () => {
      try {
        const response = await api.getAllByType(2, currentPage.value, pageSize.value);
    ...
    const submitEdit = async () => {
      try {
        ...
        const response = await api.updateSubject(2, requestData);
        if (response.code === 200) {
